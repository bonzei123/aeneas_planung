...
**Ein Verein:** ein Standard-Postfach, **5 Funktionsadressen als Aliasse** auf dasselbe Fach (`info@`, `vorstand@`, `praevention@`, …) — Limit Standard **50 Aliasse** je eigener Domain, 5 liegen locker drin. **ca. 4 € netto/Monat**, kein Silber. Zammad holt **ein** IMAP-Konto und macht nur aus `help@` (später nach `To:` in Gruppen) Tickets; die anderen Aliasse bleiben im Fach. DMARC-`rua` nicht auf dieses Fach. Fünf volle Postfächer (5×4 €) nur, wenn jemand die Fächer getrennt in Thunderbird braucht — tut ihr laut Planung nicht.
